...
export const PATTERNS: CryptoPattern[] = [
  {
    ...
    regex:
      /KeyPairGenerator\.getInstance\(\s*"RSA|generateKeyPair(?:Sync)?\(\s*['"]rsa|Crypto\.PublicKey\.RSA|hazmat[\w.]*\basymmetric\b[\w.]*\brsa\b|\brsa\.generate_private_key|RSA_generate_ke[y]|rsa\.GenerateKey|RSACryptoServiceProvide[r]|Cipher\.getInstance\(\s*"RSA/,
  ...
  },
  {
    ...
    regex:
      /createECDH\(|KeyPairGenerator\.getInstance\(\s*"EC"|generateKeyPair(?:Sync)?\(\s*['"]ec['"]|\becdsa\.GenerateKey|\bECDsa\b|\bsecp(?:256|384|521)[kr]1\b|\bprime256v1\b|hazmat[\w.]*\bec\b.*generate_private_key|ec\.generate_private_key/,
    quantumVulnerable: true,
  ...
  },
  {
    ...
    algorithm: 'DSA',
    primitive: 'signature',
    regex: /KeyPairGenerator\.getInstance\(\s*"DSA"|\bdsa\.generate_private_key|\bDSACryptoServiceProvider\b/,
    quantumVulnerable: true,
    recommendation: PQC_SIG,
  },
  {
    ...
    algorithm: 'MD5',
    primitive: 'hash',
    regex: /createHash\(\s*['"]md5|hashlib\.md5|MessageDigest\.getInstance\(\s*"MD5|\bmd5\.New\(|MD5CryptoServiceProvide[r]/i,
  ...
  },
  {
    ...
    algorithm: 'SHA-1',
    primitive: 'hash',
    regex: /createHash\(\s*['"]sha1|hashlib\.sha1|MessageDigest\.getInstance\(\s*"SHA-?1|\bsha1\.New\(|SHA1CryptoServiceProvide[r]/i,
    quantumVulnerable: false,
    recommendation: HASH_UP,
  },
  {
